/**
* If not stated otherwise in this file or this component's LICENSE
* file the following copyright and licenses apply:
*
* Copyright 2020 RDK Management
*
* Licensed under the Apache License, Version 2.0 (the "License");
* you may not use this file except in compliance with the License.
* You may obtain a copy of the License at
*
* http://www.apache.org/licenses/LICENSE-2.0
*
* Unless required by applicable law or agreed to in writing, software
* distributed under the License is distributed on an "AS IS" BASIS,
* WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
* See the License for the specific language governing permissions and
* limitations under the License.
**/

#include <rdkshellchannel.h>
#include <rdkshelldata.h>
#include <iostream>
#include <string>
#include <unistd.h>

using namespace RdkShell;

void displayResponse(std::map<std::string, RdkShellData>& response)
{
  bool firstparam = true;
  std::map<std::string, RdkShellData>::iterator iter = response.begin();
  std::cout << std::endl;
  std::cout << "{";
  while (iter != response.end())
  {
      std::string word = iter->first;
      if (!firstparam)
        std::cout << ",";
      firstparam = false;
      std::cout << word << ":";
      RdkShellData& param = iter->second;
      if (param.dataTypeIndex() == typeid(uint8_t))
      {
        std::cout << param.toUnsignedInteger8();
      }
      else if(param.dataTypeIndex() == typeid(uint32_t))
      {
        std::cout << param.toUnsignedInteger32();
      }
      else if(param.dataTypeIndex() == typeid(uint64_t))
      {
        std::cout << param.toUnsignedInteger64();
      }
      else if(param.dataTypeIndex() == typeid(int8_t))
      {
        std::cout << param.toInteger8();
      }
      else if(param.dataTypeIndex() == typeid(int32_t))
      {
        std::cout << param.toInteger32();
      }
      else if(param.dataTypeIndex() == typeid(int64_t))
      {
        std::cout << param.toInteger64();
      }
      else if(param.dataTypeIndex() == typeid(bool))
      {
        std::cout << param.toBoolean();
      }
      else if(param.dataTypeIndex() == typeid(std::string))
      {
        std::cout << param.toString();
      }
      else if(param.dataTypeIndex() == typeid(float))
      {
        std::cout << param.toFloat();
      }
      else if(param.dataTypeIndex() == typeid(double))
      {
        std::cout << param.toDouble();
      }
      iter++;
  }
  std::cout << "}";
  std::cout << std::endl;
}

void displayEvent(std::string& name, std::vector<std::map<std::string, RdkShellData>>& eventData)
{
  std::cout << "event received " << std::endl;
  std::cout << " { name : " << name << " - [ ";
  for (int i=0; i<eventData.size(); i++)
  {
    displayResponse(eventData[i]);
  }
  std::cout << " ] " << std::endl;
}

class EventListener:public RdkShellEventListener
{
  public:
    void onEvent(std::string name, std::vector<std::map<std::string, RdkShellData>>& eventData)
    {
      displayEvent(name, eventData);
    }

    void onApiError(std::string name)
    {
      std::cout << "onApiError event received " << name << std::endl;
    }
};

int main()
{
  std::string method = "createDisplay";
  EventListener listener;
  RdkShellChannel::instance()->initialize();

  method = "onAnimation";
  RdkShellChannel::instance()->registerForEvent(method, (RdkShellEventListener*)&listener);
  method = "onApiError";
  RdkShellChannel::instance()->registerForEvent(method, (RdkShellEventListener*)&listener);

  std::map<std::string, RdkShellData> requestParams;
  std::map<std::string, RdkShellData> response;

  // ##################################### TEST 1
/*
  method = "createDisplay";
  requestParams["client"] = "test_display";
  response.clear();
  bool ret = RdkShellChannel::instance()->callMethod(method, requestParams);
  std::cout << "callmethod for create display result - " << ret << std::endl;
*/

  // ##################################### TEST 2
  requestParams.clear();
  response.clear();
  method = "launchApplication";
  requestParams["client"] = "test_display";
  requestParams["uri"] = "westeros_test";
  requestParams["mime"] = "application/native";
  bool ret = RdkShellChannel::instance()->callMethod(method, requestParams);
  std::cout << "callmethod for launchApplication result - " << ret << std::endl;

  // ##################################### TEST 3
  requestParams.clear();
  response.clear();
  method = "setBounds";
  requestParams["client"] = "test_display";
  requestParams["x"] = 0;
  requestParams["y"] = 0;
  ret = RdkShellChannel::instance()->callMethod(method, requestParams);
  std::cout << "callmethod for setbounds result - " << ret << std::endl;

  // ##################################### TEST 4
  requestParams.clear();
  response.clear();
  method = "setScale";
  requestParams["client"] = "test_display";
  requestParams["sx"] = 1.5;
  ret = RdkShellChannel::instance()->callMethod(method, requestParams);
  std::cout << "callmethod for setscale result - " << ret << std::endl;

  // ##################################### TEST 5
  requestParams.clear();
  response.clear();
  method = "getScale";
  requestParams["client"] = "test_display";
  ret = RdkShellChannel::instance()->callMethodWithResult(method, requestParams, response);
  std::cout << "callmethod for getscale result - " << ret << response.size() << std::endl;
  displayResponse(response);

  // ##################################### TEST 6
  requestParams.clear();
  response.clear();
  method = "getBounds";
  requestParams["client"] = "test_display";
  ret = RdkShellChannel::instance()->callMethodWithResult(method, requestParams, response);
  std::cout << "callmethod for getbounds result - " << ret << response.size() << std::endl;
  displayResponse(response);
  std::cout << std::endl; 
   
  // sleeping here to start the app from command line
  //sleep(15);

  // ##################################### TEST 7
  method = "addAnimation";
  requestParams.clear();
  response.clear();
  requestParams["client"] = "test_display";
  requestParams["duration"] = 10;
  requestParams["x"] = 10;
  requestParams["y"] = 10;
  requestParams["sx"] = 1.0;
  requestParams["sy"] = 1.0;
  ret = RdkShellChannel::instance()->callMethod(method, requestParams);
  std::cout << "callmethod for add animation result - " << ret << std::endl;

  timespec ts;
  clock_gettime(CLOCK_MONOTONIC, &ts);
  double startTime = ts.tv_sec + ((double)ts.tv_nsec/1000000000);
  double totalElapsed = 0;
  
  // ################################### LOOK FOR EVENTS
  // perform some animation during this sleep ad check for events
  while (true)
  {
    RdkShellChannel::instance()->processMessages();
    usleep(30000);
    clock_gettime(CLOCK_MONOTONIC, &ts);
    totalElapsed = (ts.tv_sec + ((double)ts.tv_nsec/1000000000)) - startTime;
    if (totalElapsed > 30)
      break;
  }

  // ##################################### TEST 8
  requestParams.clear();
  response.clear();
  method = "kill";
  requestParams["client"] = "test_display";
  ret = RdkShellChannel::instance()->callMethod(method, requestParams);
  std::cout << "callmethod for kill result - " << ret << std::endl;

  method = "onAnimation";
  RdkShellChannel::instance()->unregisterEvent(method, &listener);
  method = "onApiError";
  RdkShellChannel::instance()->unregisterEvent(method, &listener);
  return 0;
}
