...
namespace RdkShell
{
    ...
    bool systemRam(uint32_t &availableKb, uint32_t& usedSwapKb)
    {
    ...
    static void evaluateMemoryUsage(uint32_t& availableKb, uint32_t& usedSwapKb, float swapIncreaseMb)
    {
    ...
    static void launchMemoryMonitorThread()
    {
        gMemoryMonitorThread = std::thread([=]()
        {
            ...
            uint32_t usedSwapKb=0, availableKb=0;
            bool ret = systemRam(availableKb, usedSwapKb);
            float usedSwapMb = 0;
            if (ret)
            {
            ...
            while (runMemoryMonitor)
            {
                ...
                if (enableRamMonitor)
                {
                    ret = systemRam(availableKb, usedSwapKb);
                    if (ret)
                    {
                        usedSwapMb = usedSwapKb/1024;
                        swap1=swap2;
                        ...
                        swap4=swap5;
                        swap5=usedSwapMb;
                        evaluateMemoryUsage(availableKb, usedSwapKb, (swap5-swap1));
                    }
                }
