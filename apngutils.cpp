...
namespace RdkShell
{
    ...
    void FrameData::blit(FrameData& b, int32_t dstLeft, int32_t dstTop,
                     int32_t dstWidth, int32_t dstHeight,
                     int32_t srcLeft, int32_t srcTop)
    {
        Rectangle srcBounds = bounds();
        Rectangle dstBounds = b.bounds();

        Rectangle srcRect(srcLeft, srcTop, srcLeft+dstWidth, srcTop+dstHeight);
        Rectangle dstRect(dstLeft, dstTop, dstLeft+dstWidth, dstTop+dstHeight);
        ...
        int32_t w = Min<int>(srcBounds.width(), dstBounds.width());
        int32_t h = Min<int>(srcBounds.height(), dstBounds.height());

        for (int32_t y = 0; y < h; y++)
        {
            Pixel *s = pixel(srcBounds.left(), y+srcBounds.top());
            Pixel *se = s + w;
